...
// task4c: do not forget the #include's! 
...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
    ...
    G4VisAttributes* visAttr = new G4VisAttributes(G4Colour::White());
    worldLog->SetVisAttributes(visAttr);
    //Task 1b.6: Set the world volume to be invisible
    ...
    */
    ...
    G4Sphere* sampleSolid = new G4Sphere("sampleSolid", //name 
                                       0.,   //inner radius
                                      rSphere,   //outer radius
                                       0,    //phi start
                                      twopi, //phi stop
                                       0,    //theta start
                                       pi);  //theta stop
    ...
    // Task 1b.2 Create the logical volume: uncomment and complete
      G4LogicalVolume* sampleLog = new G4LogicalVolume(sampleSolid, //solid
                                                       matCu, //material 
						       "sampleLog"); //name
      G4VisAttributes* red = new G4VisAttributes(G4Colour::Red()); //pick a color
      red->SetVisibility(true);
      red->SetForceSolid(true);
      sampleLog->SetVisAttributes(red); //attach it to the logical volume

    //3) physical volume
    // Task 1b.3 Create the physical volume: uncomment and complete
      G4VPhysicalVolume* samplePhys = new G4PVPlacement(nullptr, //rotation matrix 
                                                        G4ThreeVector(0,0,0), //null vector (0,0,0) 
                                                   sampleLog, //logical volume to be placed
                                                   "samplePhys", //name of the detector (a string you like)
                                                   worldLog, //logival volume of the mother
                                                     false,
                                                         0); //copynumber
    ...
    */
    ...
    G4Material* matNaI = nullptr; // initially it is a null pointer

    //Task 1a.1: uncomment and fill!

      matNaI = new G4Material("NaI", //its name
    ...
    G4double casingZCenter = sampleToFaceDistance + casingHeight/2.;
    ...
    G4ThreeVector casingCenter(0,0,casingZCenter); 
    G4VPhysicalVolume* casingPhys = new G4PVPlacement(nullptr, //rotation
                                                      casingCenter, //translation
    ...
    G4Tubs* naiSolid = new G4Tubs("NaISolid", //name
                                  0, //inner radius
				  NaIDiameter/2, //outer radius
                                  NaIHeight/2, //half height
				  0, // starting phi
                                  twopi); //end phi

    G4LogicalVolume* naiLog = new G4LogicalVolume(naiSolid, //solid
                                                  matNaI, //material
                                                  "NaILV"); //name
    ...
    G4VPhysicalVolume* naiPhys = new G4PVPlacement(nullptr, //rotation
                                                   G4ThreeVector(0,0,0), // null translation
                                                   naiLog,//logical volume
						   "NaI", //name
                                                    casingLog, // mother volume
                                                    false,0);
